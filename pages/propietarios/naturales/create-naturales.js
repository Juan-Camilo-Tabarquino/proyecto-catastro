...
export default function CreatePropietarioNatural (){
    ...
    const { direccion, email, telefono, tipo_documento, numero_documento, nombre, apellidos, predio, onInputChange} = useForm(nuevoPropietario);
    const { predios,startListPredios } = usePrediosStore();
    const { startCreatePropietarioNatural } = usePropietariosStore();
    
    const onSubmit = async(e) => {
        e.preventDefault();
        if( numero_documento < 0 || numero_documento.trim() === '' || nombre.trim() === '' || tipo_documento.trim() === '' || apellidos.trim() === ''  || email.trim() === '' || telefono.trim() === '' || telefono < 0 || direccion.trim() === '' || predio === 0 || predio === ""){
            Swal.fire('Error','Todos los campos son requeridos para crear un terreno.','error');
        }else{
            await startCreatePropietarioNatural({direccion,telefono,email,tipo_documento,numero_documento,nombre,apellidos,predio});
        }
    }
